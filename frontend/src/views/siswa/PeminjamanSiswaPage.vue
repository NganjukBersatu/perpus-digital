...
const userId = computed(() => props.siswa?.id || userFromStorage.id || null)
...
async function fetchPeminjaman() {
  ...
  if (!userId.value) {
    error.value = 'Data user tidak ditemukan. Silakan login ulang.'
  ...
  try {
    const res = await fetch(`${API_BASE}/dashboard-siswa/peminjaman-aktif`, {
      headers: authHeaders()
    })
...
<template>
  <div class="page">
    <div class="page-header">
      ...
      <p class="muted">Daftar buku yang sedang kamu pinjam saat ini.</p>
    </div>
          ...
          <table class="tabel">
            ...
            <tbody>
              <tr v-for="item in daftarTampil" :key="item.id">
                ...
                <td data-label="Jatuh Tempo">{{ formatTanggal(item.batasKembali) }}</td>
